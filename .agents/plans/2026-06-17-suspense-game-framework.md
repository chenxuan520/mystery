...
- 2026-06-19 02:34：修复用户反馈的“嫌疑人 / NPC 对话纯属胡言乱语、会编造案件里根本不存在的人物（如截图里的‘林雨桐’）、连有哪些嫌疑人都不知道”的问题。根因排查：`src/chat/suspect-chat.ts` 的 `buildSystemPrompt` 从最初提交起就只塞了当前角色自己的信息，从未把案件世界观、其他角色列表、调查节点等上下文给到角色（对比 `src/chat/hint-master.ts` 的提示官 prompt 一直有完整上下文）；这不是被某次提交改没了，而是一直缺，之前能用只是模型偶尔自圆其说。本轮重写 `buildSystemPrompt`，把角色 system prompt 扩成分区块结构：1）案件世界观（标题 / 开场叙述 / 公开摘要 / 场景设定 / 当前局面 / 事件导火索 / 已知紧张关系 / 玩家目标 / 死者）；2）案件全部人物（每个嫌疑人的身份 / 关系 / 表面动机 / 对外不在场说法，每个 NPC 的身份 / 关系 / 为什么值得关注，并标出“你自己”）；3）案件现场与可调查地点（列出全部调查节点，已调查的附带 discovery，未调查的标注“玩家尚未调查”）；4）角色自身身份与说话风格；5）角色内部信息（knows / hides，以及从 `solution.hiddenRelationships`、`solution.keyContradictions` 中筛出的、文本里点到本角色名字的隐藏关系与关键矛盾）；6）真凶专属内部视角（仅真凶可见：真实动机 / 作案手法 / 作案计划 / 真实时间线 / 制造的误导，并强约束绝不能直接自曝）；7）按真凶 / 非真凶嫌疑人 / NPC 三类派生出的“你这场对话的目标与行为方式”（对话 agenda，从已有字段推导，不改 schema，旧归档案件直接生效）；8）当前博弈态势（玩家已调查节点数与已查结果）；9）回复规则（强化“只能提及上述列出的角色、不得编造案件中不存在的人物 / 地点 / 证据”）。新增 `summarizeSuspect / summarizeNpc / summarizeAllCharacters / findRelevantHiddenRelationships / findRelevantContradictions / buildCharacterAgenda` 等辅助函数，并把最终拼接改成按非空区块用空行分隔，避免 NPC（无真凶区块）出现多余空行。本轮按用户实际诉求采用“只改 prompt 推导 agenda”的方案，未改 `src/case/schema.ts` / `src/case/generator.ts`，因此对全部历史归档案件立即生效、无需重新生成。fresh evidence：多次执行 `npm run build`、`npm test`（保持 15 个测试文件、60 条测试全部通过）；并用 `npx tsx` 加载真实归档案件《双面陶俑》，对真凶（顾怀瑾）与 NPC（陈馆长）分别渲染了完整 system prompt，确认两类角色都能看到完整案件世界观、全部人物列表（含自己标记）、6 个调查节点（已查 1 个带 discovery、其余标“尚未调查”）、各自的 knows/hides、涉及自身的隐藏关系与关键矛盾、以及对应的对话目标区块；真凶额外能看到真实动机 / 手法 / 计划 / 时间线 / 误导，NPC 则不含真凶区块。用户确认热加载已生效、无需重启 30001。文档同步：`README.md` 当前形态新增“嫌疑人 / NPC 对话会拿到完整案件上下文”说明。当前剩余风险：本轮是 prompt 级修复，角色“见到的东西”仍依赖案件生成时 `knows/hides` 写得是否充分；如果后续想让“对话任务 / 角色亲历见闻”成为案件数据里的显式结构化字段，需要再单独评估改 `schema + generator + quality + reviewer` 并处理旧归档迁移。

- 2026-06-19 11:58：按用户“增强趣味性，1/2/3 都做”的要求，落地三个玩法功能（Web 优先，CLI 保证不报错、原指认照常可用）。#1 线索对质：`src/chat/suspect-chat.ts` 新增 `buildConfrontationDirective` 与 `buildSuspectMessages` 的可选 `confrontNode` 参数，玩家出示某条已查实线索时，会在 user 轮前注入“玩家正拿出一条已查实线索与你正面对质”的强约束 system 指令，要求角色正面回应该具体证据（戳中隐瞒就紧张辩解/有限松口，无关就借机引向他人），但不得否认线索存在或改写真相；`streamSuspectReply`/`generateSuspectReply` 同步透传。#2 指认带推理评分 + #3 误判后果：`src/judgement/judge.ts` 新增 `evaluateAccusation`（用 play 网关 `completeJson` 对照真凶/动机/手法/关键矛盾/隐藏关系，输出 0–100 评分、verdict、命中点、漏掉点、点评，并生成 2–4 句结局旁白——指认对给落网收尾、指认错给真凶脱身/冤枉他人的戏剧后果），`JudgementResult` 增加可选 `deduction`/`consequence`，并新增确定性兜底 `buildFallbackConsequence`；旧 `judgeAccusation`/`revealSolution` 保持兼容。`src/web/server.ts`：chat 入口接收 `confrontNodeId` 并解析为节点透传；accuse 入口接收 `reasoning` + `citedNodeIds`，先出确定性判定再调用 `evaluateAccusation` 附加评分与结局，评估失败时降级到兜底结局，保证不卡死。前端 `src/web/static/app.js`：对话页为嫌疑人/NPC 增加“出示线索对质”下拉+按钮（复用流式聊天，抽出 `runChatTurn` 供 `sendChat` 与新 `confrontWithClue` 共用）；指认改为 `promptAccusationDetails` 表单（勾选线索 + 写推理，SweetAlert2，带无弹窗降级）；结案结果页新增 `renderConsequenceHtml`（结局/误判后果）与 `renderDeductionHtml`（评分卡：分数分档、命中/漏掉点、点评）；`deduction`/`consequence` 一并纳入 local cache 的 `sanitizeCachedJudgement` 与 snapshot，刷新后可恢复。`src/web/static/styles.css` 增加对质行、指认表单、评分卡、结局区与对错描边样式。测试：扩展 `tests/judge.test.ts`（`evaluateAccusation` 用假网关验证分数钳制/清洗与正确错误两路、`buildFallbackConsequence`），新增 `tests/suspect-chat.test.ts`（角色完整上下文、真凶/非真凶/NPC 区块差异、对质指令注入与否）。fresh evidence：`npm run build`、`npm test`（现为 16 个测试文件、69 条全部通过）、`node --check src/web/static/app.js`；用真实模型对归档案件《双面陶俑》验证三条链路——正确且有理的指认得 78 分并给出真凶落网旁白、瞎猜错误指认得 5 分并给出真凶销毁证据脱身的后果、对质真凶时其辩称“仿品充数”并反咬血指纹；又在临时端口 30056 起真实 Web 服务跑通 from-archive→investigate→confront(200)→accuse（返回 correct/score/consequence）端到端，验证后已关闭临时实例，未触碰用户的 30001。当前剩余风险：accuse 现在多一次模型调用，结案会比之前慢几秒；评分/结局质量随模型波动，已有确定性兜底但兜底结局较朴素；本轮按范围只做了 Web，CLI 仍是原始一键指认、无评分/对质。

- 2026-06-19 12:20：把对质 / 指认评分 / 误判后果三个玩法补到 CLI，并顺手排查了一条用户反馈的日志。日志排查：用户提到控制台出现 `Cached selected text: 当天晚上你有` 这类日志要求删除；用 `rg --no-ignore --hidden` 全量搜索整个仓库与 `node_modules`，确认该字符串在本项目代码与依赖里都不存在（只命中 TS `lib.dom.d.ts` 的无关注释），本项目唯一的前端日志是受 `window.__mysteryDebugToConsole` 开关控制的 `[Mystery Cache Debug]`，且只记录 `draftKeys` 键名、不会打印草稿原文，因此这条日志不是本项目产生的（更像 IDE / 浏览器扩展的“缓存选中文本”行为），无法在本仓库删除。CLI 实现：修改 `src/cli/run.ts`，`handleChat` 在对话对象是嫌疑人/相关人物且已有已查线索时，每轮提供“自由提问 / 出示线索对质”菜单，选对质后从已查线索里挑一条，拼成出示话术并通过 `generateSuspectReply` 的新 `confrontNode` 参数传给模型；`handleAccusation` 新增 `gateway` 入参，指认确认后让玩家写一句推理，调用 `evaluateAccusation`（失败回退 `buildFallbackConsequence`），并把 `printJudgement` 重写为 `printJudgementResult`，在真相还原前先打印结局/误判后果、破案评分、命中/漏掉点与点评；`gameLoop` 调用处补传 `gateway`。前端无改动（Web 三功能上一轮已完成），但本轮顺手把 Web 对话页的“出示线索对质”行改成始终可见：没查到线索时显示禁用提示“先去左侧调查节点查到线索”，避免用户以为功能没做。另确认并向用户说明：`npm run web` 在启动时一次性把 `app.js`/`index.html`/后端模块读进内存，之前用户“点指认没反应/看不到对质”是因为运行的是改动前启动的旧实例，已用后台方式在 30001 重启到最新代码（curl 校验 `/app.js` 含 `confront-row`）。fresh evidence：`npm run build`、`npm test`（保持 16 个测试文件、69 条通过）；在 vm 沙箱里实跑 `accuseSelected` 确认 Web 指认无报错、正确 POST `{suspectId,reasoning,citedNodeIds}` 并写回带评分/结局的 judgement；用真实模型 + 隔离 DATABASE_PATH 跑 CLI 重玩：归档载入 / 各级菜单正常；完整指认流程拿到“结局 + 破案评分 48/100 + 点评 + 真相还原”（《镜湖遗书》，约 18.7s 真实模型耗时）；对质子菜单（自由提问 / 出示线索对质）在调查到线索后正确出现。已知限制：非 TTY 下用管道驱动多级菜单需要给输入加 `sleep` 间隔，否则 readline 会因 stdin 提前 EOF 错位（这是测试驱动方式的限制，不是 CLI 逻辑问题）；CLI 指认评分同样多一次模型调用、会比之前慢十几秒。
